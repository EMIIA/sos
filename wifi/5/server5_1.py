...
class MapboxHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        try:
            # Static files - защищаем JSON стили
            if self.path.startswith("/static/"):
                file_path = os.path.join(STATIC_DIR, self.path[8:])
                
                # Блокируем прямой доступ к JSON стилям
                if self.path.endswith('.json') and 'moscow_style_2.json' in self.path:
            ...
            # Vector tiles - защищаем PBF тайлы
            ...
            if match:
                ...
                if tile_path:
                    # Проверяем Referer - разрешаем только с нашего домена
                    referer = self.headers.get('Referer', '')
                    if not referer.startswith('https://map.emiia.keenetic.pro'):
                    ...
                    self.send_header("Access-Control-Allow-Origin", "https://map.emiia.keenetic.pro")
            ...
            # API endpoint для стилей - проксируем JSON
            if self.path == "/api/style":
                style_path = os.path.join(STATIC_DIR, "moscow_style_2.json")
                if os.path.isfile(style_path):
                    ...
                    self.send_header("Cache-Control", "public, max-age=3600")
                    self.send_header("Access-Control-Allow-Origin", "*")
                    self.send_compressed(style_data, "application/json", ".json")
                    return
                self.send_error(404)
                return

            # Main page - модифицируем HTML чтобы использовала API для стилей
            if self.path == "/":
                # Создаем модифицированный HTML с API endpoint для стилей
                modified_html = HTML_CONTENT.decode('utf-8').replace(
                    "style: '/static/moscow_style_2.json'",
                    "style: '/api/style'"
                ).encode('utf-8')
                ...
                self.send_header("Content-Length", str(len(modified_html)))
                self.end_headers()
...
if __name__ == "__main__":
    ...
    print("Protection: JSON styles and PBF tiles are protected")
