...
<body>
    ...
    <script>
        ...
        // Всегда используем один стиль - упрощаем
...
def get_font_path(font_name, font_range):
    """Returns font path if exists, else None"""
    # Декодируем имя шрифта
    font_name = font_name.replace("%20", " ")
    
    # Ищем в разных местах
    font_paths = [
        os.path.join(STATIC_DIR, "fonts", font_name, f"{font_range}.pbf"),
        os.path.join(BASE_DIR, "fonts", font_name, f"{font_range}.pbf"),
        os.path.join("fonts", font_name, f"{font_range}.pbf"),
    ]
...
class MapboxHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        try:
            ...
            # Fonts handling - НОВЫЙ БЛОК
            font_match = re.match(r"^/fonts/(.+?)/(\d+-\d+)\.pbf$", self.path)
            if font_match:
                font_name, font_range = font_match.groups()
                font_path = get_font_path(font_name, font_range)
